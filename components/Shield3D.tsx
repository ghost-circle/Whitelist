...
const ShieldModel = ({ url, scale, position, isWhite, multiColor }: { url: string; scale: number; position: [number, number, number]; isWhite?: boolean; multiColor?: boolean }) => {
  ...
  useFrame((state) => {
    if (meshRef.current && multiColor) {
      ...
      const currentScene = sceneRef.current;
      currentScene.rotation.x += (targetRotationX - currentScene.rotation.x) * 0.05;
      currentScene.rotation.y += (targetRotationY - currentScene.rotation.y) * 0.05;
    }
  });
...
const Shield3D: React.FC = () => {
  return (
    <div style={{ width: '128px', height: '128px' }}>
      <Canvas camera={{ position: [0, 0, 4], fov: 45 }} shadows style={{ width: '100%', height: '100%' }} resize={{ scroll: false, debounce: { scroll: 0, resize: 0 } }}>
        <ambientLight intensity={0.3} color="#1a1a2e" />
        <directionalLight position={[5, 5, 5]} intensity={0.6} color="#a78bfa" castShadow />
        <pointLight position={[0, 2, 2]} intensity={1.5} color="#7c3aed" distance={10} decay={2} />
        <pointLight position={[-2, -1, 1]} intensity={0.5} color="#8b5cf6" distance={8} decay={2} />
        <Suspense fallback={null}>
          <ShieldModel url="/shield.glb" scale={1} position={[0, -1, 0]} multiColor={true} />
        </Suspense>
      </Canvas>
    </div>
  );
